...
function loadConfigFile(name, cb) {
  fs.readFile('.forever', 'utf8', function(err, data) {
    if (err) {
      return forever.log.error('Forever cannot find .forever file.');
    }
    ...
    try {
      ...
      forever.log.error('Error parsing: .forever');
      return;
    }
...
//
// ### function start
// Loads a config file in .forever
//
app.cmd('start', cli.startConfig = function () {
  loadConfigFile('.forever', function(config) {
    var file = config.script,
        options = config.options || {};

    forever.log.info('Forever using config from .forever');
